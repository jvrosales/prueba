...
import ec.gob.ambiente.enlisy.speciescatalog.model.SpecieCatalogDocument;
 ...
 */
@Stateless
public class SpecieCatalogDocumentFacade extends AbstractFacade<SpecieCatalogDocument, Integer>{

	public SpecieCatalogDocumentFacade() {
		super(SpecieCatalogDocument.class, Integer.class);		
	}
	
	 @EJB(lookup = Constant.ALFRESCO_SERVICE_BEAN)
	    private AlfrescoServiceBean alfrescoServiceBean; 
	
	public boolean saveAlfrescoBdd(SpecieCatalogDocument document,String userName)
	{		
		try
		{			
			if(document.getContent()!=null&&(document.getScdoStatus()==null||document.getScdoStatus()))
			{
				String nombreCarpeta=document.getScdoIdProcess();				
				String folderId=guardarCarpeta(nombreCarpeta);
				String alfrescoId=guardarArchivo(document.getContent(),document.getScdoName(), folderId,nombreCarpeta);
				document.setScdoAlfrescoId(alfrescoId);
				String nombreDocumento=document.getScdoName().toLowerCase();
				...
				document.setScdoName(nombreDocumento);
			}
			
			if(document.getScdoId()==null)
			{
				document.setScdoUserCreate(userName);
				document.setScdoDateCreate(new Date());
				document.setScdoStatus(true);
				create(document);
				return true;
			}
			else
			{
				document.setScdoUserUpdate(userName);
				document.setScdoDateUpdate(new Date());
				edit(document);
				return true;
			}		
